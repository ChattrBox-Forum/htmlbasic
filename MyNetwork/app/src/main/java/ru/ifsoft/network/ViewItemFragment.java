...
import ru.ifsoft.network.dialogs.CommentUserActionDialog;
...
public class ViewItemFragment extends Fragment implements Constants, SwipeRefreshLayout.OnRefreshListener, CommentInterface, TagClick {
    ...
    TextView mItemFeelingTitle, mItemCity, mItemCountry, mItemAuthor, mItemUsername, mItemMode, mItemPost, mItemTimeAgo, mItemLikesCount, mItemCommentsCount, mItemRePostsCount, mRePostItemAuthor, mRePostItemUsername, mRePostItemTimeAgo, mRePostItemPost;
    ImageView mRePostItemImg;
    CircularImageView mItemAuthorPhoto, mRePostItemAuthorPhoto, mItemFeelingIcon;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mItemFeelingTitle = (TextView) mListViewHeader.findViewById(R.id.itemFeelingTitle);
        mItemFeelingIcon = (CircularImageView) mListViewHeader.findViewById(R.id.itemFeelingIcon);
    ...
    public void updateItem() {
        ...
        if (item.getFeeling() == 0) {
            ...
            mItemFeelingTitle.setVisibility(View.GONE);

        } else {

            mItemFeelingIcon.setVisibility(View.VISIBLE);
            mItemFeelingTitle.setVisibility(View.VISIBLE);

            ImageLoader imageLoader = App.getInstance().getImageLoader();

            imageLoader.get(Constants.WEB_SITE + "feelings/" + Integer.toString(item.getFeeling()) + ".png", ImageLoader.getImageListener(mItemFeelingIcon, R.drawable.mood, R.drawable.mood));
        }
        ...
        if (item.getPostType() == POST_TYPE_DEFAULT) {
        ...
        } else if (item.getPostType() == POST_TYPE_PHOTO_UPDATE) {

            mItemPost.setVisibility(View.VISIBLE);
            mItemPost.setText(item.getFromUserFullname() + " " + getActivity().getString(R.string.label_updated_profile_photo));
            ...
            mItemPost.setVisibility(View.VISIBLE);
            mItemPost.setText(item.getFromUserFullname() + " " + getActivity().getString(R.string.label_updated_cover_photo));
        }
...
//                                Toast.makeText(ViewItemActivity.this, response.toString(), Toast.LENGTH_SHORT).show();
    ...
    public void onPostEdit(final int position) {
        ...
        if (item.getPostType() == POST_TYPE_DEFAULT) {
            ...
            i.putExtra("imgUrl", item.getImgUrl());
            i.putExtra("feeling", item.getFeeling());
            startActivityForResult(i, ITEM_EDIT);

        } else {

            Toast.makeText(getActivity(), getString(R.string.msg_system_post), Toast.LENGTH_SHORT).show();
        }
    }
